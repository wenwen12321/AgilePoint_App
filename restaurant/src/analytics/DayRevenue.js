...
export default class DayRevenue extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            // isNeedUpdate: false,
            ...
            // auth: null,
            // userID: null,
            dayRevenue: props.dayRevenue,
            // storeID: null,
            historyHourHeader: ["時間", "營業額"],
            historyHourRevenue: props.historyHourRevenue,
            historyHourRevenueForTable: [],
        }
    }
    ...
    componentDidMount() {
        var arr = [];
        for (var i = 0; i < this.state.historyHourRevenue.date.length; i++) {
            arr.push([this.state.historyHourRevenue.date[i], this.state.historyHourRevenue.revenue[i]]);
        }
        this.setState({
            historyHourRevenueForTable: arr
        });
        console.log("day:"+this.state.historyHourRevenue);
    }
            ...
            >
                ...
                <View style={{ marginVertical: 10, margin: 5 }}>
                    <CardView
                        ...
                        cornerRadius={5}
                    >
                        <Text style={{ margin: 10, fontSize: 20 }}>今日每小時營業額</Text>
                        <View>
                            <LineCharts
                                fromZero
                                label={this.state.historyHourRevenue.date}
                                data={this.state.historyHourRevenue.revenue}
                                rotation={35}
                        ...
                        <View style={{ margin: 10 }}>
                            <Table borderStyle={{ borderWidth: 2, borderColor: '#c8e1ff' }}>
                                <Row data={this.state.historyHourHeader} style={styles.head} textStyle={styles.text} />
                                <Rows data={this.state.historyHourRevenueForTable} textStyle={styles.text} />
                            </Table>
                        </View>
                    </CardView>
                </View>
